    ...
    for (const key in todoDictionary) {
      if (todoDictionary.hasOwnProperty(key)) {
        ...
        if ( element["title"].toLowerCase().startsWith(value.toLowerCase())) {
          let todo_sequence = document.createElement('<div class="grid_todo_sequence_number">1</div>')
          todo_sequence.classList("grid_todo_sequence_number");
          todo_sequence.innerHTML = "1";
          todo_sequence.className = "grid_todo_sequence_number";


          todo_Data +=
            '<div class="todo_item">' +
            "1" +
            '<input style="postion:relative; float:left" type="checkbox" value="' +
            element["completed"] +
            '"></input>' +
